...
import { Category, Product, ProductVariant, Branch, ApiResponse } from '@/types';
import { RoleGuard } from '@/components/RoleGuard';
import { ChevronRight, Plus, RefreshCw, Folder, Package, Layers, Building2, CheckCircle2, XCircle, Edit2, Eye } from 'lucide-react';
...
interface ProductWithVariants extends Product {
  ...
  branchAssignments?: { branch_id: number; branch_name: string; is_available: number }[];
}
...
export default function SetupFlowPage() {
    ...
    try {
      ...
      if (categoriesResponse.success) {
        ...
        const uniqueCategories = Array.from(
          new Map(categoriesData.map((cat: Category) => [cat.category_name, cat])).values()
        );
        ...
        const categoriesWithProducts = await Promise.all(
          uniqueCategories.map(async (category: Category) => {
              try {
                ...
                const allCategoriesResponse: ApiResponse<{ data: Category[] }> = await makeAuthenticatedRequest(
                  `/categories?store_id=${user.store_id}`,
                  {},
                  true,
                  user.store_id,
                ...
                if (allCategoriesResponse.success) {
                  const allCats = allCategoriesResponse.data.data || allCategoriesResponse.data || [];
                  categoryIds = allCats
                    .filter((c: Category) => c.category_name === category.category_name)
                ...
                const productsPromises = categoryIds.map(async (catId: number) => {
                ...
                products = await Promise.all(
                  productsData.map(async (product: Product) => {
                    try {
                      ...
                      const branchAssignments = await getProductBranchAssignments(product.product_id);
                      
                      // Get variants
                      const variantsResponse: ApiResponse<{ data: ProductVariant[] }> = await makeAuthenticatedRequest(
                      ...
                      return { ...product, branchAssignments: [], variants: [] };
                    }
                  })
                );
              }
              ...
              console.error(`Error loading products for category ${category.category_id}:`, err);
        ...
        if (categoriesWithProducts.length > 0) {
          setExpandedCategories(new Set([categoriesWithProducts[0].category_id]));
        }
      }
    } catch (err: any) {
  ...
  const getProductBranchAssignments = async (productId: number) => {
    try {
      const response: ApiResponse<{ data: { branch_id: number; branch_name: string; is_available: number }[] }> = 
  ...
  const handleToggleProductBranch = async (productId: number, branchId: number, currentStatus: number) => {
    try {
      ...
      if (response.success) {
        ...
        throw new Error(response.message || 'Failed to update branch assignment');
      }
    } catch (err: any) {
    ...
    <RoleGuard requiredPermissions={['manage_categories', 'manage_products']} fallback={
      ...
      <div className={`min-h-screen ${bgClass} py-8 px-4 sm:px-6 lg:px-8`}>
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className={textClass}>
                Manage your master categories, products, and variants with branch-wise assignment
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <SummaryCard
              ...
              value={categories.reduce((sum, cat) => 
                sum + (cat.products?.reduce((pSum, prod) => pSum + (prod.variants?.length || 0), 0) || 0), 0)}
              color="purple"
              theme={isDark}
            />
            <SummaryCard
        ...
        <div className={`${cardClass} border rounded-xl shadow-sm p-6`}>
          {categories.length === 0 ? (
            <EmptyState theme={isDark} onAdd={() => router.push('/categories/new')} />
          ) : (
            <div className="space-y-4">
              {categories.map((category) => (
                <CategorySection
                  ...
                  onAddProduct={() => router.push(`/products/new?category_id=${category.category_id}`)}
                  onViewProduct={(productId) => router.push(`/products/${productId}`)}
                  onEditProduct={(productId) => router.push(`/products/${productId}/edit`)}
                  onAddVariant={(productId) => router.push(`/product-variants/new?product_id=${productId}`)}
                  theme={isDark}
                />
              ))}
            </div>
...
function SummaryCard({ icon, label, value, color, theme }: {
  ...
  const colorClasses = {
    ...
    green: isDark ? 'bg-green-900/30 border-green-700/50' : 'bg-green-50 border-green-200',
    purple: isDark ? 'bg-purple-900/30 border-purple-700/50' : 'bg-purple-50 border-purple-200',
    orange: isDark ? 'bg-orange-900/30 border-orange-700/50' : 'bg-orange-50 border-orange-200'
  };
...
function CategorySection({
  ...
  isExpanded: boolean;
  onToggle: () => void;
  onToggleProductBranch: (productId: number, branchId: number, currentStatus: number) => Promise<void>;
  onAddProduct: () => void;
  ...
  return (
    <div className={`border rounded-lg ${isDark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-gray-50'}`}>
      {/* Category Header */}
      <button
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-4 flex-1">
          <Folder size={24} className={isDark ? 'text-blue-400' : 'text-blue-600'} />
          <div className="text-left flex-1">
            <h3 className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {category.category_name}
            </h3>
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
              {category.description || 'No description'} • {products.length} product{products.length !== 1 ? 's' : ''}
            </p>
          </div>
        ...
        <ChevronRight
          size={20}
          className={`${isDark ? 'text-gray-400' : 'text-gray-600'} transition-transform ${isExpanded ? 'rotate-90' : ''}`}
        />
      </button>
        ...
        <div className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} p-4`}>
          {products.length === 0 ? (
            <div className="text-center py-8">
              <p className={`${isDark ? 'text-gray-400' : 'text-gray-600'} mb-4`}>No products in this category</p>
              <button
...
function ProductSection({
  ...
  branches: Branch[];
  onToggleBranch: (productId: number, branchId: number, currentStatus: number) => Promise<void>;
  ...
  const branchAssignments = product.branchAssignments || branches.map(b => ({
    branch_id: b.branch_id,
    branch_name: b.branch_name,
    is_available: 0
  }));
  ...
  return (
    <div className={`border rounded-lg ${isDark ? 'border-gray-700 bg-gray-800/30' : 'border-gray-200 bg-white'}`}>
      <div className="p-4">
        {/* Product Header */}
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-2">
              <Package size={20} className={isDark ? 'text-green-400' : 'text-green-600'} />
            ...
            <p className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
              Base Price: ₹{parseFloat(product.base_price?.toString() || '0').toFixed(2)}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                isDark ? 'hover:bg-blue-900/40 text-blue-400' : 'hover:bg-blue-100 text-blue-600'
              }`}
              title="View"
            >
              <Eye size={18} />
            ...
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                isDark ? 'hover:bg-indigo-900/40 text-indigo-400' : 'hover:bg-indigo-100 text-indigo-600'
              }`}
              title="Edit"
            >
              <Edit2 size={18} />
            ...
            {branchAssignments.map((assignment) => {
              ...
              return (
                <button
                  ...
                  onClick={() => onToggleBranch(product.product_id, assignment.branch_id, assignment.is_available)}
        ...
        <div className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} pt-4`}>
          ...
          {variants.length === 0 ? (
            <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'} italic`}>
              No variants yet
            </p>
          ) : (
                ...
                >
                  <div>
                    ...
                    <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                      ₹{parseFloat(variant.variant_price?.toString() || '0').toFixed(2)}
                    </p>
                  </div>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                    variant.is_active === 1
