...
import { Plus, Search, Edit3, Trash2, Eye, Moon, Sun, ChevronDown, ChevronUp, Folder } from 'lucide-react';
...
export default function CategoriesPage() {
  const [categoriesByBranch, setCategoriesByBranch] = useState<Map<number, { branch_name: string; categories: CategoryWithBranch[] }>>(new Map());
      ...
      try {
        ...
        if (response.success) {
          ...
          if (selectedBranch) {
            ...
            // Group by branch
            const newMap = new Map<number, { branch_name: string; categories: CategoryWithBranch[] }>();
    ...
    try {
      ...
      if (response.success) {
        ...
        newMap.forEach((branchData) => {
          branchData.categories = branchData.categories.filter(cat => cat.category_id !== categoryId);
        });
        setCategoriesByBranch(newMap);
      } else {
        ...
        <div className="max-w-7xl mx-auto px-8 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className={`${t.textSecondary} text-sm mt-1`}>Organize and manage product categories by branch</p>
            </div>
        ...
        <div className={`${t.cardBg} rounded-xl border ${t.cardBorder} p-5 mb-8 shadow-sm hover:shadow-md transition-shadow duration-300`}>
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex-1">
              <label className={`block text-sm font-medium ${t.text} mb-2`}>Search Categories</label>
            ...
            <div className="w-full md:w-auto">
              <label className={`block text-sm font-medium ${t.text} mb-2`}>Filter by Branch</label>
              <select
        ...
        {loading ? (
          <div className={`${t.cardBg} rounded-xl border ${t.cardBorder} shadow-sm p-12`}>
            <div className="flex flex-col items-center justify-center gap-4">
              ...
              <p className={`${t.textSecondary} font-medium`}>Loading categories...</p>
            </div>
          </div>
        ) : categoriesByBranch.size === 0 ? (
          <div className={`${t.cardBg} rounded-xl border ${t.cardBorder} shadow-sm p-12 text-center`}>
            <Folder size={48} className={`mx-auto ${isDarkMode ? 'text-slate-600' : 'text-slate-300'} mb-3`} />
            <p className={`${t.textSecondary} font-medium`}>No categories found</p>
            <p className={`${t.textTertiary} text-sm mt-1`}>Get started by creating a new category.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {Array.from(categoriesByBranch.entries()).map(([branchId, { branch_name, categories }]) => {
              const filtered = filteredCategories(categories);
              if (selectedBranch && selectedBranch !== branchId) return null;
              
              return (
                <div key={branchId} className={`${t.cardBg} rounded-xl border ${t.cardBorder} shadow-sm overflow-hidden transition-all duration-300`}>
                  {/* Branch Header */}
                  <button
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <h2 className={`text-lg font-semibold ${t.text}`}>{branch_name}</h2>
                        <p className={`text-xs ${t.textTertiary}`}>{filtered.length} categor{filtered.length !== 1 ? 'ies' : 'y'}</p>
                      </div>
                    </div>
                    ...
                    <div className={`border-t ${t.cardBorder}`}>
                      {filtered.length === 0 ? (
                        <div className={`px-6 py-8 text-center`}>
                          <p className={t.textTertiary}>No categories in this branch</p>
                        </div>
                      ) : (
                        <ul className={`divide-y ${t.cardBorder}`}>
                          {filtered.map((category, idx) => (
                            <li key={category.category_id} className={`${t.categoryRow} transition-all duration-200`}>
                              <div className="px-6 py-4">
                                <div className="flex items-center justify-between mb-2">
                                  <div className="flex-1">
                                    <p className={`text-sm font-semibold ${t.text}`}>{category.category_name}</p>
                                    <p className={`text-xs ${t.textTertiary} mt-1`}>{category.description || 'No description'}</p>
                                  </div>
                                ...
                                <div className="flex items-center justify-between mt-3 pt-3 border-t border-opacity-20">
                                  <div className={`text-xs ${t.textTertiary}`}>
                                    {category.total_products || 0} product{(category.total_products || 0) !== 1 ? 's' : ''}
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <button
                                      onClick={() => window.location.href = `/categories/${category.category_id}`}
                                      className={`p-1.5 rounded-lg transition-colors duration-200 ${isDarkMode ? 'hover:bg-blue-900/40 text-blue-400' : 'hover:bg-blue-100 text-blue-600'}`}
                                      title="View"
                                    >
                                      <Eye size={16} />
                                    </button>
                                    <button
                                      onClick={() => window.location.href = `/categories/${category.category_id}/edit`}
                                      className={`p-1.5 rounded-lg transition-colors duration-200 ${isDarkMode ? 'hover:bg-indigo-900/40 text-indigo-400' : 'hover:bg-indigo-100 text-indigo-600'}`}
                                      title="Edit"
                                    >
                                      <Edit3 size={16} />
                                    </button>
                                    <button
                                      ...
                                      className={`p-1.5 rounded-lg transition-colors duration-200 ${isDarkMode ? 'hover:bg-red-900/40 text-red-400' : 'hover:bg-red-100 text-red-600'}`}
                                      title="Delete"
                                    >
                                      <Trash2 size={16} />
                                    </button>
